...
const databaseUrl = 'postgres://db.example.test:5432/restaurant';

describe('buildTypeOrmConfig', () => {
  it('builds a production-safe PostgreSQL configuration', () => {
    const config = buildTypeOrmConfig(databaseUrl, [Restaurant]);
...
describe('AppDataSource', () => {
  ...
  it('wires the service entity without connecting or enabling implicit schema changes', () => {
    ...
    expect(entities.map((entity) => entity.name)).toEqual(['Restaurant']);
